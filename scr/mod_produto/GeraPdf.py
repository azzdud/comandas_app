...
class PDF(FPDF):
    def header(self):
        self.image("scr/static/erro404.jpg", 10, 8, 20)
    ...
    def listaTodos(self):
        ...
        try:
            ...
            for row in result[0]:
                ...
                pdf.cell(
                    115, 5, str(row["nome"]) + " - " + str(row["descricao"]), 0, 0, "L"
                )
                pdf.cell(15, 5, str(row["valor_unit"]), 0, 0, "L")
                # converte de string base64 para imagem
                img = Image.open(
                    BytesIO(
                        base64.b64decode(
                            re.sub("^data:image/.+;base64,", "", row["foto"])
                        )
                    )
                )
