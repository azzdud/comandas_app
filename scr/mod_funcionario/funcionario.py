...
def insert():
    try:
        ...
        response = requests.post(
            ENDPOINT_FUNCIONARIO, headers=HEADERS_API, json=payload
        )
        result = response.json()
...
def formEditFuncionario():
    try:
        ...
        response = requests.get(
            ENDPOINT_FUNCIONARIO + id_funcionario, headers=HEADERS_API
        )
        result = response.json()
...
def edit():
    try:
        ...
        response = requests.put(
            ENDPOINT_FUNCIONARIO + id_funcionario, headers=HEADERS_API, json=payload
        )
        result = response.json()
