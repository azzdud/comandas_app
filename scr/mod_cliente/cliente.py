...
def edit():
    try:
        ...
        response = requests.put(
            ENDPOINT_CLIENTE + id_cliente, headers=HEADERS_API, json=payload
        )
        result = response.json()

        if response.status_code == 200:
            print("Cliente atualizado com sucesso!")
            # Faça o que for necessário com os dados do 'result'
        else:
            print(
                f"Erro na atualização do cliente. Código de status: {response.status_code}"
            )
            print(f"Detalhes do erro: {result['msgErro']}")
